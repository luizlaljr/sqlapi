...
module.exports = {
    async index (req, res) {
        try {
            ...
            const users = await User.sequelize.query(`SELECT view_base.id, view_base.antique, view_base.post, view_base.name, view_base.condition,  view_base.date_condition, view_base.operationality, view_base.activity, view_base.project, SUM(amount) as amount ,SUM(income) * view_base.factor + Sum(transport) * 95 as income 
            FROM view_base 
